...
export class Reports5Component implements OnInit {
  //////theme variables//////
  ...
  constructor(private dataSharingService: Datashare1Service,
    private ngxService: NgxUiLoaderService, private theme: ThemeService,
    private router: Router) {

    this.dataSharingService.isrep5_gauge.subscribe(value => {
      if (value != 0) {
        this.callme();
      }
  ...
  callme() {
    ...
    this.theme.currenttheme.subscribe(themename => this.themechange = themename)
      this.element = document.querySelector('.' + this.themechange)
      this.colorVal = getComputedStyle(this.element).getPropertyValue('--accent-color');
    this.needleValue = parseInt(localStorage.getItem("rep5_needleValue"));
    this.options = {
      hasNeedle: true,
